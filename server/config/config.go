...
func applyConfigFile(filename string) error {
	...
	// and if it did, resolve it relative the config file
	// directory itself
	tmpCfg := &Config{}
...
func Load() {
	...
	applyConfigFile(filepath.Join(user.HomeDir, ".title-server/config.json"))

	if CFG.Secret == "" {
		log.Fatal("You must define a randomized 'secret' string in your config file. See config.json.example")
	}
}
